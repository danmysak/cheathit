...
ALLOWED_COMPONENTS = [GROUP, STUDENT, PROBLEM, ATTEMPT]
COMPONENT_DELIMITER = '/'
DEFAULT_PATH = STUDENT
COMPONENT_DESCRIPTION = f'{", ".join(map(repr, ALLOWED_COMPONENTS))}'f' delimited with {repr(COMPONENT_DELIMITER)}'
...
def parse_path(path: str) -> tuple[str, ...]:
    ...
    if not all(component in ALLOWED_COMPONENTS for component in components):
        raise ValueError(f'Expected {COMPONENT_DESCRIPTION}, got {repr(path)}')
    if STUDENT not in components:
        raise ValueError(f'Expected {repr(STUDENT)} among the path components, got {repr(path)}')
...
def parse_ratio(value: str) -> float:
    ...
    if ratio is None or not 0.0 <= ratio <= 1.0:
        raise ValueError(f'Expected a number between 0.0 and 1.0, got {value}')
    return ratio
...
def run() -> None:
    ...
    parser.add_argument('root', type=Path, help='Root directory with submissions')
    ...
    parser.add_argument('--min-ratio', type=parse_ratio, nargs='?', default=DEFAULT_MIN_RATIO,
                        help='Minimum ratio of the number of shared tokens'
                             ' to the number of tokens in the longer submission')
    ...
    for student_match in analysis.student_matches:
        ...
        for index, submission_match in enumerate(student_match.submission_matches):
            ...
            print(numeral(submission_match.common_tokens, 'common token')
                  + ' / ' + numeral(len(submission_match.longest_ngram), 'token') + ' in the longest common match: ')
